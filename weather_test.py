...
import requests
def get_dku_weather():
    API_KEY = st.secrets["OPENWEATHER_API_KEY"]  # 🔑 Streamlit Secrets에 저장해둔 키 사용
    lat, lon = 37.3219, 127.1265  # 단국대 죽전 캠퍼스 좌표
    ...
    # 주요 정보 파싱
    weather = res["weather"][0]["main"].lower()   # 날씨 상태 (예: rain, clear, clouds 등)
    description = res["weather"][0]["description"] # 상세 설명
    temp = res["main"]["temp"]
...
# 🧭 stage 초기화 (맨 처음엔 idle 상태)
if "stage" not in st.session_state:
    st.session_state.stage = "idle"


# 🌤️ 날씨는 앱 켜질 때 한 번만 가져오기
if ui_ctx["weather_tag"] is None or ui_ctx["temp_bucket"] is None:
    weather_tag, temp_bucket = get_dku_weather()
    ui_ctx["weather_tag"] = weather_tag
    ui_ctx["temp_bucket"] = temp_bucket
...
def save_choice_to_neo4j(choice_row: dict):
    ...
    + 현재 날씨 정보(weather_tag, temp_bucket)도 함께 저장
    """
    user_id = st.session_state["user_id"]
    ui_ctx  = st.session_state.context
    ...
    # 🔹 날씨 정보 (사전에 get_dku_weather()로 context에 넣어둔다고 가정)
    weather_tag  = ui_ctx.get("weather_tag")      # 예: "비", "맑음", "흐림"
    temp_bucket  = ui_ctx.get("temp_bucket")      # 예: "추움", "선선함", "따뜻함", "더움"
    ...
    // 유저와 관계 및 유저의 최근 날씨 정보도 업데이트
    MERGE (u)-[:MADE]->(c)
    SET u.last_weather_tag  = $weather_tag,
        u.last_temp_bucket  = $temp_bucket,
        u.last_choice_at    = datetime()
