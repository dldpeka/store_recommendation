# croling_naverplace
네이버 플레이스(지도) 상세페이지의 **리뷰(방문자 리뷰)**를 자동으로 수집하는 파이썬 크롤러입니다. 

Selenium으로 동적 로딩/‘더보기’ 버튼 클릭/스크롤을 처리하고, BeautifulSoup으로 DOM을 파싱해 작성자 닉네임, 본문, 태그/아이콘, 이미지 URL 등을 추출해 JSON으로 저장합니다.
